...
def experiment(args):
    ...
    try:
        ...
        args.buffer_size = int(args.buffer_size*n_train_samples) * 2
        ...
        for epoch in range(num_epochs):
            ...
            #         X_tgt, _ = tgt_data_iter.next()  # Target Domain Labels should not be used
                ...
                if net.C.module.use_gumbel:
                    _, src_gen_logits_out, _ = net.discriminator(X_src_gen)
                    _, tgt_gen_logits_out, _ = net.discriminator(X_tgt_gen)
                else:
                    _, src_gen_logits_out = net.discriminator(X_src_gen)
                    _, tgt_gen_logits_out = net.discriminator(X_tgt_gen)

                # src_gen_prob = F.softmax(src_gen_logits_out, dim=1)  # torch.Size([64, 10])
                # src_gen_log_prob = F.log_softmax(src_gen_logits_out, dim=1)  # torch.Size([64, 10])
                # # tgt_gen_prob = F.softmax(tgt_gen_logits_out, dim=1)  # torch.Size([64, 10])
                # # tgt_gen_log_prob = F.log_softmax(tgt_gen_logits_out, dim=1)  # torch.Size([64, 10])
                #
                src_adv_loss = F.log_softmax(src_gen_logits_out, dim=1).mean(dim=1).mean() * net.mone
                # # src_adv_loss = F.log_softmax(src_gen_logits_out, dim=1).sum()  # * net.mone
                tgt_adv_loss = F.log_softmax(tgt_gen_logits_out, dim=1).mean(dim=1).mean() * net.mone  #  * net.mone
                # # tgt_adv_loss = F.log_softmax(tgt_gen_logits_out, dim=1).sum()  #  * net.mone
                # src_adv_loss = src_gen_prob * src_gen_log_prob
                # src_adv_loss = src_adv_loss.sum(dim=1).mean()  # * net.mone  # maximize entropy for -ve samples
                # # tgt_adv_loss = tgt_gen_prob * tgt_gen_log_prob
                # # tgt_adv_loss = tgt_adv_loss.sum(dim=1).mean()  # * net.mone  # maximize entropy for -ve samples
                # bcs_adv_loss_fn = nn.BCEWithLogitsLoss().cuda()
                # target_ones = torch.new_ones(src_gen_logits_out.size)
                # src_adv_loss = bcs_adv_loss_fn(src_gen_logits_out, )

                # lambda_ssl = ramp_sup_weight_in_list[0]
                # lambda_sul = lambda_tul = ramp_unsup_weight_in_list[0]
                total_classifier_loss = lambda_ssl * ramp_sup_weight_in_list[0] * src_sup_loss + lambda_tul * ramp_unsup_weight_in_list[0] * tgt_unsup_loss + lambda_sal * src_adv_loss + lambda_tal * tgt_adv_loss  # Total Encoder + Classifier loss for Enc + Class training
