...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.conv1_3 = nn.Conv2d(96, 96, (3, 3), stride=2, padding=1)
        self.conv1_3_bn = nn.BatchNorm2d(96)
        # self.pool1 = nn.MaxPool2d((2, 2))
        self.drop1 = nn.Dropout(0.2)

        self.conv2_1 = nn.Conv2d(192, 192, (3, 3), stride=1, padding=1)
        ...
        self.conv2_3 = nn.Conv2d(192, 192, (3, 3), stride=2, padding=1)
    ...
    def forward(self, x):
        ...
        x = F.relu(self.conv2_1_bn(self.conv2_1(x)), negative_slope=.2)
        x = F.relu(self.conv2_2_bn(self.conv2_2(x)), negative_slope=.2)
        x = F.relu(self.conv2_3_bn(self.conv2_3(x)), negative_slope=.2)
        x = self.drop2(x)

        x = F.relu(self.conv3_1_bn(self.conv3_1(x)), negative_slope=.2)
        x = F.relu(self.conv3_2_bn(self.conv3_2(x)), negative_slope=.2)
        x = F.relu(self.conv3_3_bn(self.conv3_3(x)), negative_slope=.2)
