    ...
    def __init__(self, root, train=True,
        ...
        if self.train:
            ...
            for fentry in self.train_list:
                f = fentry[0]
                file = os.path.join(self.root, self.dataset_name, self.processed_folder, self.base_folder, f)
                fo = open(file, 'rb')
                if sys.version_info[0] == 2:
            ...
            train_mask = self.train_labels != -1

            self.train_data = self.train_data[train_mask]
            self.train_labels = list(self.train_labels[train_mask])
            ...
            # Remove all samples from skipped classes
            test_mask = self.test_labels != -1

            self.test_data = self.test_data[test_mask]
            self.test_labels = list(self.test_labels[test_mask])
