...
# from data_prep.msda_preprocessed_amazon_dataset import get_datasets
from model.build_network import Network
...
def experiment(args):
    ...
    log_num = 0
    log_file = logs_dir + '/' + hostname + '_' + machinename + '_' + args.exp + '_' + str(log_num) + '_' + args.epoch_size + '_ts' + '.txt'

    # Setup logfile to store output logs
    if log_file is not None:
        while os.path.exists(log_file):
            log_num += 1
            log_file = '{0}/{3}_{1}_{2}_{4}_ts.txt'.format(args.logs_dir, args.exp, log_num, hostname, args.epoch_size)
        # return
    ...
    ckpt_dir = os.path.join(absolute_base_path, args.ckpt_dir, hostname + '_' + machinename + '_' + args.exp) + '_' + str(log_num) + '_' + args.epoch_size + '_ts'
    ...
    try:
        ...
        # Get datasets
        src_domain, tgt_domain = args.exp.split('_')

        d_src_train, d_src_test = get_data(args, domain=src_domain)
        #d_tgt_test, d_tgt_train = get_data(args, domain=tgt_domain)
        d_tgt_train, d_tgt_test = get_data(args, domain=tgt_domain)

        log_str += log('\nSRC : {}: train: count={}, test: count={}'.format(
            d_src_train.domain.upper(), d_src_train.__len__(),
            d_src_test.__len__()))
        log_str += log('TGT : {}: train: count={}, test: count={}'.format(
            d_tgt_train.domain.upper(), d_tgt_train.__len__(),
            d_tgt_test.__len__()))
        ...
        pin_memory = True
        if use_sampler:
            d_src_labels = d_src_train.labels.numpy()

            labels, counts_src = np.unique(d_src_labels, return_counts=True)
            ...
            sampler_weights_src = weights_src[d_src_labels]
            ...
            d_tgt_labels = d_tgt_train.labels.numpy()
            labels, counts_tgt = np.unique(d_tgt_labels, return_counts=True)
            ...
            sampler_weights_tgt = weights_tgt[d_tgt_labels]
        ...
        for epoch in range(num_epochs):
            ...
            # if use_sampler:
            for i, ((_, _), (X_tgt, y_tgt)) in enumerate(zip(l_src_train, l_tgt_train)):
            ...
            #         X_tgt, _ = tgt_data_iter.next()  # Target Domain Labels should not be used
                ...
                tgt_sup_loss = classification_criterion(tgt_logits_out, y_tgt)  # Loss 1 : Supervised loss  # torch.Size([64, 10])
                ...
                total_classifier_loss = lambda_tsl * ramp_sup_weight_in_list[0] * tgt_sup_loss # + lambda_sul * ramp_unsup_weight_in_list[0] * src_unsup_loss + lambda_tul * ramp_unsup_weight_in_list[0] * tgt_unsup_loss  + lambda_sal * src_adv_loss + lambda_tal * tgt_adv_loss  # Total Encoder + Classifier loss for Enc + Class training

                ### Add loss and loss weights to tensorboard logs
                # net.writer.add_scalar('classifier_loss/sup/src', src_sup_loss.item(), n_iter)
                net.writer.add_scalar('classifier_loss/sup/tgt', tgt_sup_loss.item(), n_iter)
            ...
            if tgt_test_acc >= best_tgt_test_acc:
                ...
                cax = plt.scatter(tsne_results[:len(src_labels_orig), 0], tsne_results[:len(src_labels_orig), 1],
                                  c=torch.tensor(src_labels_orig).numpy(), alpha=1.0,
                                  cmap=plt.cm.get_cmap("jet", d_src_test.n_classes), marker='*',  # 'x',
                                  label='SRC - {} - #{}'.format(d_src_test.domain.upper(), len(src_labels_orig)))
                cax = plt.scatter(tsne_results[len(src_labels_orig):, 0], tsne_results[len(src_labels_orig):, 1],
                                  c=torch.tensor(tgt_labels_orig).numpy(), alpha=1.0,
                                  cmap=plt.cm.get_cmap("jet", d_tgt_test.n_classes), marker='+',  # '+',
                                  label='TGT - {} - #{}'.format(d_tgt_test.domain.upper(), len(tgt_labels_orig)))
                plt.legend(loc='upper left')
                # plt.legend(loc='upper right')
                fig.colorbar(cax, extend='min', ticks=np.arange(0, d_src_test.n_classes))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp', choices=['books_dvd', 'books_electronics', 'books_kitchen',
                                          'dvd_books', 'dvd_electronics', 'dvd_kitchen',
                                          'electronics_books', 'electronics_dvd', 'electronics_kitchen',
                                          'kitchen_books', 'kitchen_dvd', 'kitchen_electronics'
                                          ], default='books_dvd', help="experiment to run ['books', 'dvd', 'electronics', 'kitchen']")
    parser.add_argument('--dataroot', type=str, default='./data/prep-amazon/amazon.pkl', help='path to dataset file')
    ...
    parser.add_argument('--buffer_size', type=int, default=600, help='length of the buffer for latent feature selection : (default=10000)')
    parser.add_argument('--batch_size', type=int, default=128, help='mini-batch size : (default=64)')
    ...
    parser.add_argument('--gpus', type=str, default='2,3', help='using gpu device id')
    ...
    parser.add_argument('--network_type', type=str, default='dade', help='type of network [se|mcd|dade]')
    parser.add_argument('--epoch_size', default='target', choices=['large', 'small', 'source', 'target'], help='epoch size is either that of the smallest dataset, the largest, the source, or the target (default=source)')
    ...
    parser.add_argument('--n_test_samples', type=int, default=500, help='number of test samples used for plotting t-SNE')
