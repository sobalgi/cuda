...
def experiment(args):
    ...
    log_num = 0
    log_file = logs_dir + '/' + hostname + '_' + machinename + '_' + args.exp + '_' + str(log_num) + '_' + args.epoch_size + '_ss_tu_su_sa' + '.txt'

    # Setup logfile to store output logs
    if log_file is not None:
        while os.path.exists(log_file):
            log_num += 1
            log_file = '{0}/{3}_{1}_{2}_{4}_ss_tu_su_sa.txt'.format(args.logs_dir, args.exp, log_num, hostname, args.epoch_size)
        # return
    ...
    ckpt_dir = os.path.join(absolute_base_path, args.ckpt_dir, hostname + '_' + machinename + '_' + args.exp) + '_' + str(log_num) + '_' + args.epoch_size + '_ss_tu_su_sa'
    ...
    try:
        ...
        X_src_noise = Normal(d_src_train.loc, d_src_train.scale)
        X_tgt_noise = Normal(d_tgt_train.loc, d_tgt_train.scale)
        ...
        for epoch in range(num_epochs):
            ...
            #         X_tgt, _ = tgt_data_iter.next()  # Target Domain Labels should not be used
                ...
                with torch.no_grad():
                    ...
                    X_src_gen = X_src_noise.sample(X_src_size).cuda()
                    # X_tgt_gen = X_tgt_noise.sample(X_tgt_batch_size).cuda()
                ...
                total_classifier_loss = lambda_ssl * ramp_sup_weight_in_list[0] * src_sup_loss + lambda_tul * ramp_unsup_weight_in_list[0] * tgt_unsup_loss + lambda_sul * ramp_unsup_weight_in_list[0] * src_unsup_loss + lambda_sal * src_adv_loss # + lambda_sul * ramp_unsup_weight_in_list[0] * src_unsup_loss + lambda_tul * ramp_unsup_weight_in_list[0] * tgt_unsup_loss  + lambda_sal * src_adv_loss + lambda_tal * tgt_adv_loss  # Total Encoder + Classifier loss for Enc + Class training
