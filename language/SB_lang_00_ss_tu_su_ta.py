...
def experiment(args):
    ...
    log_num = 0
    log_file = logs_dir + '/' + hostname + '_' + machinename + '_' + args.exp + '_' + str(log_num) + '_' + args.epoch_size + '_ss_tu_su_ta' + '.txt'

    # Setup logfile to store output logs
    if log_file is not None:
        while os.path.exists(log_file):
            log_num += 1
            log_file = '{0}/{3}_{1}_{2}_{4}_ss_tu_su_ta.txt'.format(args.logs_dir, args.exp, log_num, hostname, args.epoch_size)
        # return
    ...
    ckpt_dir = os.path.join(absolute_base_path, args.ckpt_dir, hostname + '_' + machinename + '_' + args.exp) + '_' + str(log_num) + '_' + args.epoch_size + '_ss_tu_su_ta'
        ...
        for epoch in range(num_epochs):
            ...
            #         X_tgt, _ = tgt_data_iter.next()  # Target Domain Labels should not be used
                ...
                with torch.no_grad():
                    ...
                    # X_src_gen = X_src_noise.sample(X_src_batch_size).cuda()
