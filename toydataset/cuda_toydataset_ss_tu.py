...
def experiment(args):
    ...
    try:
        ...
        plt.savefig('{}/plots/src_{}_seed_{:04d}_ss_tu_original.png'.format(net.args.ckpt_dir, src_name, seed))
        ...
        plt.savefig('{}/plots/src_{}_seed_{:04d}_ss_tu_original_acc.png'.format(net.args.ckpt_dir, src_name, seed))
        # net.writer.add_figure('{}_tsne'.format(args.exp), fig, global_step=(epoch * len(tgt_labels_pred)) + j)
        plt.tight_layout()
        net.writer.add_figure('src_{}_seed_{:04d}_ss_tu_original_acc'.format(src_name, seed), fig, global_step=0)
        ...
        #             break  # stop training after 100
            ...
            if tgt_test_acc >= best_tgt_test_acc:
                ...
                plt.axis('off')
                os.system('mkdir -p {}/plots'.format(net.args.ckpt_dir))
                plt.savefig('{}/plots/tsne/src_{}_seed_{:04d}_ss_tu_tsne_epoch{:03d}.png'.format(net.args.ckpt_dir, src_name, seed, epoch))
                ...
                plt.savefig('{}/plots/tsne_acc/src_{}_seed_{:04d}_ss_tu_tsne_acc_epoch{:03d}.png'.format(net.args.ckpt_dir, src_name, seed, epoch))
                # net.writer.add_figure('{}_tsne'.format(args.exp), fig, global_step=(epoch * len(tgt_labels_pred)) + j)
                plt.tight_layout()
                net.writer.add_figure('src_{}_seed_{:04d}_ss_tu_tsne'.format(src_name, seed), fig, global_step=epoch)
            ...
            plt.axis('off')
            os.system('mkdir -p {}/plots'.format(net.args.ckpt_dir))
            plt.savefig('{}/plots/contour/src_{}_seed_{:04d}_ss_tu_contour_epoch{:03d}.png'.format(net.args.ckpt_dir, src_name, seed, epoch))
            plt.title('{} : ACCURACIES : EPOCH {}\nS_TRN= {:5.2%}, T_TRN= {:5.2%}\nS_TST= {:5.2%}, T_TST= {:5.2%}'.format(
                args.exp.upper(), epoch, src_train_acc / (l_src_train.__len__() * batch_size),
                tgt_train_acc / (l_tgt_train.__len__() * batch_size), src_test_acc / len(d_src_test),
                tgt_test_acc / len(d_tgt_test)), fontsize=8)
            plt.savefig('{}/plots/contour_acc/src_{}_seed_{:04d}_ss_tu_contour_acc_epoch{:03d}.png'.format(net.args.ckpt_dir, src_name, seed, epoch))
            # net.writer.add_figure('{}_tsne'.format(args.exp), fig, global_step=(epoch * len(tgt_labels_pred)) + j)
            plt.tight_layout()
            net.writer.add_figure('src_{}_seed_{:04d}_ss_tu_contour'.format(src_name, seed), fig, global_step=epoch)
