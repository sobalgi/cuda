...
def experiment(args):
    ...
    log_num = seed
    log_file = logs_dir + '/' + hostname + '_' + machinename + '_' + args.exp + '_' + str(log_num) + '_' + args.epoch_size + '_ss_tu_ta' + '.txt'

    # Setup logfile to store output logs
    if log_file is not None:
        while os.path.exists(log_file):
            log_num += 1
            log_file = '{0}/{3}_{1}_{2}_{4}_ss_tu_ta.txt'.format(args.logs_dir, args.exp, log_num, hostname, args.epoch_size)
        # return
    ...
    ckpt_dir = os.path.join(absolute_base_path, args.ckpt_dir, hostname + '_' + machinename + '_' + args.exp) + '_' + str(log_num) + '_' + args.epoch_size + '_ss_tu_ta'
    args.ckpt_dir = ckpt_dir
    ...
    try:
        ...
        plt.savefig('{}/plots/src_{}_seed_{:04d}_ss_tu_ta_original.png'.format(net.args.ckpt_dir, src_name, seed))
        ...
        plt.savefig('{}/plots/src_{}_seed_{:04d}_ss_tu_ta_original_acc.png'.format(net.args.ckpt_dir, src_name, seed))
        # net.writer.add_figure('{}_tsne'.format(args.exp), fig, global_step=(epoch * len(tgt_labels_pred)) + j)
        plt.tight_layout()
        net.writer.add_figure('src_{}_seed_{:04d}_ss_tu_ta_original_acc'.format(src_name, seed), fig, global_step=0)
        ...
        #             break  # stop training after 100
            ...
            #         X_tgt, _ = tgt_data_iter.next()  # Target Domain Labels should not be used
                ...
                total_classifier_loss = lambda_ssl * ramp_sup_weight_in_list[0] * src_sup_loss + lambda_tul * ramp_unsup_weight_in_list[0] * tgt_unsup_loss + lambda_tal * tgt_adv_loss # + lambda_sul * ramp_unsup_weight_in_list[0] * src_unsup_loss + lambda_tal * tgt_adv_loss  # Total Encoder + Classifier loss for Enc + Class training
            ...
            # mat = torch.cat((src_mat, src_mat, src_train_mat), dim=0)
            # pred_mat = torch.cat((src_pred_mat, src_pred_mat, src_train_pred_mat), dim=0)
            # # metadata = src_metadata + src_metadata
            # pred_prob = src_pred_prob + src_pred_prob + src_train_pred_prob
            ...
            if tgt_test_acc >= best_tgt_test_acc:
                ...
                plt.axis('off')
                os.system('mkdir -p {}/plots'.format(net.args.ckpt_dir))
                plt.savefig('{}/plots/tsne/src_{}_seed_{:04d}_ss_tu_ta_tsne_epoch{:03d}.png'.format(net.args.ckpt_dir, src_name, seed, epoch))
                ...
                plt.savefig('{}/plots/tsne_acc/src_{}_seed_{:04d}_ss_tu_ta_tsne_acc_epoch{:03d}.png'.format(net.args.ckpt_dir, src_name, seed, epoch))
                # net.writer.add_figure('{}_tsne'.format(args.exp), fig, global_step=(epoch * len(tgt_labels_pred)) + j)
                plt.tight_layout()
                net.writer.add_figure('src_{}_seed_{:04d}_ss_tu_ta_tsne'.format(src_name, seed), fig, global_step=epoch)
            ...
            plt.axis('off')
            os.system('mkdir -p {}/plots'.format(net.args.ckpt_dir))
            plt.savefig('{}/plots/contour/src_{}_seed_{:04d}_ss_tu_ta_contour_epoch{:03d}.png'.format(net.args.ckpt_dir, src_name, seed, epoch))
            plt.title('{} : ACCURACIES : EPOCH {}\nS_TRN= {:5.2%}, T_TRN= {:5.2%}\nS_TST= {:5.2%}, T_TST= {:5.2%}'.format(
                args.exp.upper(), epoch, src_train_acc / (l_src_train.__len__() * batch_size),
                tgt_train_acc / (l_tgt_train.__len__() * batch_size), src_test_acc / len(d_src_test),
                tgt_test_acc / len(d_tgt_test)), fontsize=8)
            plt.savefig('{}/plots/contour_acc/src_{}_seed_{:04d}_ss_tu_ta_contour_acc_epoch{:03d}.png'.format(net.args.ckpt_dir, src_name, seed, epoch))
            # net.writer.add_figure('{}_tsne'.format(args.exp), fig, global_step=(epoch * len(tgt_labels_pred)) + j)
            plt.tight_layout()
            net.writer.add_figure('src_{}_seed_{:04d}_ss_tu_ta_contour'.format(src_name, seed), fig, global_step=epoch)
...
if __name__ == '__main__':
    ...
    ps aux | grep 'localhost' | awk '{print $2}' | xargs kill -9
